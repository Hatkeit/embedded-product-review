...
| 항목 | 내용 |
|---|---|
| 문서번호 | PS-MI-250W-220V **Rev. C** |
| 시스템 | PV 1장 → **2상 인터리브 플라이백(180°)** → **공유 언폴딩 브리지** → 계통 |
| 계통 | 220 V / 60 Hz 단상 (한국) |
| 정격 | AC 250 W (**순시 피크 500 W**) |
| 입력 | PV 모듈 250 W급, Vmp 18–42 V, **Voc(STC) ≤ 43.4 V** |
| 입력 커패시터 | **63 V 2,700 µF × 5 = 13,500 µF 실장 확정** |
| 입력 절대최대 | **50 V** (63 V 전해의 79.4 % — 적합) |
| 검증 | 설계계산 `calc_mi.py` **10/10 PASS** · 동작검증 `sim_mi.py` **35/35 PASS** |
...
> **실장 확정 : 63 V 2,700 µF 전해 × 5 = 13,500 µF (수동 방식)**
> 설계 최소요구(10 %pp 기준 7,129 µF)의 **1.89 배** — 여유는 있으나 넉넉하지는 않습니다.
> Rev.A 에서 검토한 능동 디커플링(11 µF 필름 + 양방향 벅부스트)은 채택되지 않았습니다.

### 4.1 정상운전 — 적합

| 항목 | 설계 최소 7,129 µF | **실장 13,500 µF** |
|---|---|---|
| 2ω 리플 | 3,042 mVpp (9.97 %) | **1,589 mVpp (5.2 %)** |
| MPPT 이용률 | 98.92 % | **99.749 %** |
| THD 영향 | — | 없음 (용량별 편차 0.01 %p) |

**전압 디레이팅 — 63 V 정격이라 여유 있음**

| 조건 | 전압 | 63 V 대비 |
|---|---|---|
| Vmp 운전 | 30.5 V | 48.4 % ✔ |
| Voc (STC) | 37.6 V | 59.7 % ✔ |
| Voc (−25 °C) | 43.6 V | 69.2 % ✔ |
| **입력정격 50 V** | 50.0 V | **79.4 % ✔** (관용기준 80 % 이내) |

→ **입력 절대최대 50 V 유지, 패널 Voc(STC) ≤ 43.4 V.**

**리플전류 — 개당 여유는 있으나 데이터시트 대조 필수**

```
120 Hz  : 5.80 A / 5 = 1.16 A/개
100 kHz : 6.28 A / 5 = 1.26 A/개  (주파수보정 1.4 적용 → 등가 0.90 A)
120 Hz 등가 합성 = 1.47 Arms/개   ← 데이터시트 정격과 비교할 값
```

2,700 µF/63 V 는 작은 캔이라 27,000 µF급 대비 리플 정격이 낮습니다.
**100 kHz 성분은 스위치 근처 세라믹 뱅크가 흡수하도록 배치**하면 전해 부담이 1.16 A 로 줄어듭니다.
ESR 25~60 mΩ 가정 시 개당 손실 62~136 mW, 자기발열 0.7~2.4 K.

**저장에너지** ½CV² @Voc(−25 °C) = **12.8 J**. SELV 이므로 감전 위험은 없으나
정비 시 단락 아크를 막기 위해 블리더는 두는 것이 좋습니다.

### 4.2 문제 — 구름 급변 시 Vpv 스윙

용량이 최소요구의 1.89 배뿐이라 **일사량 급변에서 PV 전압이 크게 흔들립니다.**

| 진폭루프 게인 | 대역 | Vpv 범위 (1000→300→1000 W/m²) | 변동 | 정격 THD |
|---|---|---|---|---|
| Rev.A 기준 (미조정) | 4.2 Hz | 25.00 ~ 36.03 V | **11.03 V** | 1.66 % |
| **6배 (Rev.C 사양)** | **25.3 Hz** | 27.73 ~ 33.89 V | **6.17 V** | 1.67 % |
| 16배 (과대) | 67.6 Hz | 28.49 ~ 33.62 V | 5.13 V | **31.75 % ✘** |

> **진폭루프 대역에는 상한이 있습니다.** 대역이 계통 주파수 60 Hz 에 근접하면
> 루프가 **라인주기 안에서 전류진폭을 변조**해 파형이 깨집니다.
> 16배(67.6 Hz)에서 THD 31.75 % / OCP 7,504회로 붕괴합니다.
> **상한은 f_line/2 = 30 Hz**, 사양값 25.3 Hz 가 그 직전입니다.
>
> 즉 **13,500 µF 로는 구름 에지에서 Vpv ±3 V 스윙을 피할 수 없습니다.**
> 다만 운전범위(18~50 V) 이탈이 없고 OCP 도 걸리지 않으며, 지속시간이 수십 ms 라
> 발전량 손실은 무시할 수준입니다. **허용 가능하나 여유는 없습니다.**

### 4.3 수명

리플 자기발열이 0.7~2.4 K 이므로 코어온도 ≈ 주위온도 + 2 K 입니다.
**부품 등급이 수명을 결정합니다.**
...
> **실장된 2,700 µF/63 V 의 온도·수명 등급을 반드시 확인하십시오.**
> 옥외 패널 배면 60 °C 기준 105 °C/5,000 h 급이면 11.2 년, 85 °C/2,000 h 급이면 1.1 년입니다.

### 4.4 참고 — Rev.A 능동 디커플링 검토 (미채택, 기록 보존)

200/400 V 스윙 기준 11.1 µF (450 V 필름 2×10 µF). 처리전력 79.6 W →
효율 96 % 기준 **−1.27 %p**, 수명 25년+. 체적·복잡도·효율을 대가로 수명을 사는 방식입니다.
...
| 진폭루프 | **kp 0.42, ki 7.2 (~25 Hz)** | 실장 13,500 µF 기준. **대역 상한 f_line/2 = 30 Hz** (4.2절) |
| Vpv 평균화 | 반주기(8.33 ms) 이동평균 | 필수 (리플 1.59 Vpp) |
...
### 6.2 라인주기 동작 검증 (`sim_mi.py`) — 35/35 PASS
...
| **구름 급변 시 Vpv 변동** | **6.17 V** (미조정 게인 11.03 V) | ≤ 7 V |
| **진폭루프 대역 상한 실증** | 16배에서 THD 31.75 % 붕괴 | f_line/2 = 30 Hz |
| **실장 뱅크 2ω 리플** | **1,589 mVpp** (해석 1,611) | ≤ 2,000 mVpp |
| **실장 뱅크 MPPT 이용률** | **99.749 %** | ≥ 99.5 % |
| **개당 리플전류 (120 Hz 등가)** | **1.47 Arms** | ≤ 2 A |
...
| R5b | 저장에너지 12.8 J | 정비 시 단락 아크 | 블리더 권장 (SELV 라 감전위험은 없음) |
| R5c | **구름 급변 시 Vpv ±3 V 스윙** | 에지에서 MPP 이탈 | 용량이 최소요구의 1.89배뿐. 진폭루프 대역 상한(30 Hz)에 걸려 더 못 줄임. 허용하되 여유 없음 |
| R5d | **2,700 µF 캔의 리플 정격** | 초과 시 수명 급락 | 개당 1.47 Arms(120 Hz 등가). **실장품 데이터시트 대조 필수**. 100 kHz 는 세라믹이 흡수하도록 배치 |
...
| Rev. | 일자 | 내용 |
|---|---|---|
| C | 2026-09-17 | **입력 전해 실장값 정정 : 60 V 27,000 µF ×5 → 63 V 2,700 µF ×5 (13,500 µF).** Rev.B 의 입력정격 48 V 하향 및 패널 Voc 41.4 V 상한을 **철회**(63 V 정격이라 50 V 는 79.4 %로 적합). 진폭루프 게인 19배→6배(KP_A 0.42/KI_A 7.2), 진폭루프 대역 상한 f_line/2 = 30 Hz 발견. 저장에너지 128→12.8 J. 검증 35/35 PASS |
| B (철회) | 2026-09-17 | 입력 전해 실장 확정(60 V 27,000 µF ×5 = 135,000 µF) 반영. 진폭루프 게인 19배 재조정(구름 급변 Vpv 변동 6.87→1.40 V), 입력정격 50→48 V 하향, 패널 Voc(STC) 상한 43.1→41.4 V, 저장에너지 128 J 안전항목 추가. 검증 32/32 PASS |
