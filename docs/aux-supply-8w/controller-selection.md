...
| 참조 | 사양 |
|---|---|
| **U1** | **TI LM5156H** (150 °C 변형, HTSSOP-14; 히컵 필요 시 LM51561H). BIAS ← Vin 직결(3.5–60 V), RT 220 k → 100 kHz, CS 100 mV ±7 %, 고정 슬로프 40 mV/T + RSL. LEB 사양은 없음 — CS RC 필터로 대응. **상세: [`lm5156h-circuit.md`](lm5156h-circuit.md)** |
| **Rcs** | **33 mΩ / 0.25 W** — 문턱 −7 %(93 mV)에서도 I_CL 2.82 A ≥ Ipk 2.38 × 1.15. (초판의 42 mΩ은 I_CL(min) 2.21 A로 전부하 미달 → 정정.) 손실 0.03 W. 저인덕턴스 필수 — 100 mV 문턱은 노이즈에 민감 |
| R_st | **불요** (BIAS 직결). Naux는 VCC 외부 급전으로 효율 개선용 선택 |
| 슬로프보상 | RSL = 0. **CCM이면 보상량 부족**(필요 15.8 mV/µs vs 고정 4 + RSL 최대 6 mV/µs) → DCM 유지가 필수 조건: Lp ±5 %, DITHOFF = VCC |
...
| # | 항목 | 이유 |
|---|---|---|
| 1 | ~~LM5156 최대 듀티 @100 kHz ≥ 0.535~~ | **확인 완료** — 데이터시트 DMAX2 90/93/96 % (RT 220 k) |
| 2 | ~~플라이백 모드 슬로프보상 설정 절차~~ | **확인 완료** — DS 9.3.7 Eq.6–9. 결론은 위 표 |
