...
## 1. "1차·2차 공통 GND, 써도 되는가" — 결론

**동작은 합니다.** 절연 플라이백의 2차 GND를 1차에 묶으면 트랜스는 **결합 인덕터**가 되고 권선비로 전압만 만듭니다. 다만 **세 가지**가 걸립니다.

**① 공통 GND 레일은 떠 있는 부하를 구동할 수 없습니다** — 이게 실질적 관문입니다.
기존 보드가 12 V를 `HGND` 기준으로 뺀 이유가 정확히 이것입니다.

언폴더 하이사이드는 **반주기(8.33 ms) 내내 ON** 이므로 부트스트랩 드룹을 계산해야 합니다.

```
드룹 = Iq x 8.33 ms / C_boot
  Iq 0.2 mA / C_boot 10 uF  ->  0.17 V   (성립)
  Iq 1.0 mA / C_boot 10 uF  ->  0.83 V   (한계)
```

**저Iq 드라이버 + 큰 부트스트랩 캡이면 성립하지만 여유가 없습니다.**

**② 계통 220 V / DC링크 400 V 측과는 절대 공통 불가** — 강화절연 위반입니다.
PV측 내부끼리 공통은 무방합니다.

**③ 1차 스위칭 리턴전류가 2차 기준을 흔듭니다** — VDDA는 별도 LDO + 스타그라운드 필수.

### 설계 반영

| 레일 | 기준 | 판단 |
|---|---|---|
| VCC_3V3 / VDDA_3V3 / VCC_5V0 | PV측 공통 GND | **무방** |
| VCC_12V0 | **S2 독립 권선** | 하이사이드/계통측이면 필수 |

**S2를 SGND에 묶으면 비절연, 띄우면 플로팅** — 배선만 바꾸면 되도록 설계했습니다(트랜스 동일).

> 절연이 불필요한 것으로 확정되면 **플라이백 대신 광입력 벅이 더 효율적**입니다.
> 플라이백을 유지하는 유일한 근거는 "12 V를 플로팅으로 뺄 수 있다" 하나입니다.
...
| **피드백** | 절연형 : TL431 + 포토커플러 / 비절연형 : 1 % 분압저항 (옵토 불요) |
...
| A6 | **절연 요구 미확정** | 1장 참조. 절연이면 TIW + 옵토, 비절연이면 직접 분압 |
