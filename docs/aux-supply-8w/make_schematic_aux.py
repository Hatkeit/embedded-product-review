...
txt(24, 54, "PS-AUX-8W Rev.A  |  입력 PV 18~50 V  |  출력 VCC_3V3 5 W / VDDA_3V3 1 W / "
...
# 피드백
block(X_D + 100, RAIL + 210, 210, 62, "피드백",
      ["절연형 : TL431 + 포토커플러", "비절연형 : 직접 분압 (옵토 불요)"], R, "#fef2f2")
wire([(X_D + 100, RAIL + 241), (X_T - 245, RAIL + 241), (X_T - 245, SWY + 42)], R, 1.5, "5,3")
...
# GND 구성 설명
block(880, YB - 4, 400, 200, "", (), R, "#fef2f2")
txt(1080, YB + 20, "1차·2차 공통 GND, 써도 되는가?", 12.5, R, "middle", "800")
lines(896, YB + 40, [
    "동작은 한다 - 트랜스가 결합인덕터가 될 뿐이다.",
    "다만 세 가지가 걸린다 :",
    "",
    "1. 공통 GND 레일은 떠 있는 부하를 못 돌린다.",
    "   언폴더 하이사이드는 반주기(8.33 ms) ON 이라",
    "   부트스트랩 드룹 = Iq x 8.33ms / Cboot.",
    "   Iq 0.2 mA / 10 uF -> 0.17 V (성립)",
    "   Iq 1 mA / 10 uF   -> 0.83 V (한계)",
    "2. 계통 220 V / DC링크 400 V 측과는 절대 불가",
    "   (강화절연 위반).  PV측 내부끼리면 무방.",
    "3. 1차 스위칭 리턴전류가 2차 기준을 흔든다.",
    "   -> VDDA 는 별도 LDO + 스타그라운드 필수",
], 9.5, K)
block(880, YB + 212, 400, 150, "", (), R, "#fff")
txt(1080, YB + 234, "결론", 12, R, "middle", "800")
lines(896, YB + 252, [
    "VCC_3V3 / VDDA_3V3 / VCC_5V0  -> 공통 GND 무방",
    "VCC_12V0                      -> S2 독립권선 유지",
    "",
    "S2 를 SGND 에 묶으면 비절연, 띄우면 플로팅.",
    "배선만 바꾸면 되도록 설계했다 (트랜스 동일).",
    "절연 불필요가 확정이면 플라이백 대신 광입력 벅이",
    "더 효율적이다 - 플라이백의 유일한 근거가 이것이다.",
], 9.5, K)
